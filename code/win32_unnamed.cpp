...
inline void Win32DrawSoundBufferMarker(
    win32_offscreen_buffer *Backbuffer, win32_sound_output *SoundOutput, float C, int PadX, int Top, int Bottom,
    DWORD Value, uint32_t Color)
{
    Assert(Value < SoundOutput->SecondaryBufferSize);
    float XReal = C * (float)Value;
    ...
    win32_sound_output *SoundOutput, float TargetSecondsPerFrame)
{
    ...
    for (int MarkerIndex = 0; MarkerIndex < MarkerCount; ++MarkerIndex)
    {
        ...
        Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom, ThisMarker->PlayCursor, 0xFFFFFF);
        Win32DrawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom, ThisMarker->WriteCursor, 0xFF0000);
    }
}
                ...
                while (GlobalRunning)
                {
                    ...
                    *NewKeyboardController = {};
                    ...
                    if (SoundIsValid)
                    {
                        ...
                        char TextBuffer[256];
                        sprintf_s(
                            TextBuffer, sizeof(TextBuffer), "LPC:%u BTL:%u TC:%u BTW:%u - PC:%u WC:%u\n",
                    ...
                    Win32DebugSyncDisplay(
                        &GlobalBackbuffer, ArrayCount(DebugTimeMarkers), DebugTimeMarkers, &SoundOutput,
